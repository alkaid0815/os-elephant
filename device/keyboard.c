...
static void intr_keyboard_handler(void) {
	...
	__attribute__((unused)) bool ctrl_down_last = ctrl_status;
	...
	if (break_code) { // 若是断码break_code(按键弹起时产生的扫描码)
		/* 由于ctrl_r和alt_r的make_code和break_code都是两字节,所以可用下面的方法取make_code,多字节的扫描码暂不处理 */
	...
	if ((scancode > 0x00 && scancode < 0x3b) || (scancode == alt_r_make) || (scancode == ctrl_r_make)) {
		...
		if (scancode < 0x0e 			//0x0e 数字'0'～'9',字符'-',字符'='
			|| scancode == 0x29 		//0x29 字符'`'
			|| scancode == 0x1a 		//0x1a 字符'['
			|| scancode == 0x1b 		//0x1b 字符']'
			|| scancode == 0x2b 		//0x2b 字符'\\'
			|| scancode == 0x27 		//0x27 字符';'
			|| scancode == 0x28 		//0x28 字符'\''
			|| scancode == 0x33 		//0x33 字符','
			|| scancode == 0x34 		//0x34 字符'.'
			|| scancode == 0x35) {	//0x35 字符'/'
				if (shift_down_last) shift = true;		// 如果同时按下了shift键
		} else {		// 默认为字母键
			if (shift_down_last && caps_lock_last) shift = false;				// 如果shift和capslock同时按下
			else if (shift_down_last || caps_lock_last) shift = true;		// 如果shift和capslock任意被按下
			else shift = false;
		}
